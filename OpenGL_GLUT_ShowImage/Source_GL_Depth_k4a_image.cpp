...
void createTexture(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture_width, texture_height, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, initTexture.get_buffer());
...
void drawBackground()
{
	...
	if (bufferIndex > 0)
	{
		...
		if (texture_id != 0)
		{
			...
			ColorizeDepthImage(outDepthFrame, DepthPixelColorizer::ColorizeBlueToRed, GetDepthModeRange(config.depth_mode), &depthTextureBuffer);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture_width, texture_height, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, depthTextureBuffer.data());
		}
	}
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		if (device.get_capture(&capture, std::chrono::milliseconds(0)))
		{
			initTexture = capture.get_depth_image();
			break;
		}
	}
